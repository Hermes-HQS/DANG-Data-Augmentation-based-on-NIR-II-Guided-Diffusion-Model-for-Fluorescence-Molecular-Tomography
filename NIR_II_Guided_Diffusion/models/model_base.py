...
class VeryBasicModel(pl.LightningModule):
    ...
    def training_step(self, batch: dict, batch_idx: int):
        opt = self.optimizers()
        opt.zero_grad()
        loss = self._step(batch, batch_idx, "train", self._step_train, opt)
        self.training_step_outputs.append(loss)
        self.manual_backward(loss)
        opt.step()
        self._step_train += 1 # =self.global_step
        # temp = self._step(batch, batch_idx, "train", self._step_train, optimizer_idx)
        return loss

    def validation_step(self, batch: dict, batch_idx: int):
        opt = self.optimizers()
        opt.zero_grad()
        loss = self._step(batch, batch_idx, "val", self._step_val, opt)
    ...
    def test_step(self, batch: dict, batch_idx: int):
        opt = self.optimizers()
        opt.zero_grad()
        loss = self._step(batch, batch_idx, "test", self._step_test, opt)
