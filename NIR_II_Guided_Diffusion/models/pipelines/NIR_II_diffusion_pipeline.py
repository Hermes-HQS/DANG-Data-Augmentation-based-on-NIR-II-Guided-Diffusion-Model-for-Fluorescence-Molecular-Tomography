...
from NIR_II_Guided_Diffusion.models import BasicModel
from NIR_II_Guided_Diffusion.utils.train_utils import EMAModel
from NIR_II_Guided_Diffusion.utils.math_utils import kl_gaussians
...
class DiffusionPipeline(BasicModel):
    def __init__(self,
                 ...
                 # Probability to drop condition during training, has only an effect for label-conditioned training
                 num_samples=4,
                 do_input_centering=True,  # Only for training
                 clip_x0=True,
                 # Has only an effect during traing if use_self_conditioning=True, import for inference/sampling
                 use_ema=False,
                 ema_kwargs={},
    ...
    def forward(self, x_t, t, condition=None, self_cond=None, guidance_scale=1.0, cold_diffusion=False,
                un_cond=None, xi=1.0, mu_a=1.0, mu_s=1.0):
        ...
        if (condition is not None) and (guidance_scale != 1.0):
            ...
            # Calculate ∇_Phi N(Phi) using autograd
            x_t.requires_grad_(True)  
            kl_loss = self.calculate_kl_divergence(x_t, condition)  
            grad_kl = torch.autograd.grad(kl_loss, x_t, create_graph=False, retain_graph=False)[0]
            x_t.requires_grad_(False)  

            # Apply the guidance formula
            s_theta = pred_uncond  
            pred = s_theta + xi * grad_kl / (mu_a + mu_s)
        ...
        if self.estimator_objective == 'x_0':
            x_t_prior, x_0 = self.noise_scheduler.estimate_x_t_prior_from_x_0(x_t, t, pred,
            ...
            x_t_prior, x_0 = self.noise_scheduler.estimate_x_t_prior_from_x_T(x_t, t, pred,
                                                                                clip_x0=self.clip_x0,
                                                                                var_scale=pred_var_scale,
                                                                                cold_diffusion=cold_diffusion)
            x_T = pred
    ...
    def calculate_kl_divergence(self, Phi, y):
        """
        Calculates the KL divergence between p_gamma(y|Phi) and U.
        This is a placeholder; you'll need to implement the actual calculation
        based on your specific distributions p_gamma and U.
        """
        # This is a dummy implementation. Replace with your actual KL divergence calculation.
        # Example: Assuming p_gamma is a Gaussian and U is a standard Gaussian:
        mean_p = Phi  # Example: Phi represents the mean of p_gamma
        logvar_p = torch.ones_like(Phi)  # Example: Assuming a fixed variance for p_gamma
        mean_u = torch.zeros_like(Phi)  # Mean of U (standard Gaussian)
        logvar_u = torch.zeros_like(Phi)  # Log variance of U (standard Gaussian)

        kl_loss = kl_gaussians(mean_p, logvar_p, mean_u, logvar_u)
        return kl_loss